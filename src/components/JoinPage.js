...
const JoinPage = ({ onGoToHome }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('from_name', `VZ Botz - ${formData.whatsapp_bot}`);
      formDataToSend.append('from_email', formData.email);
      formDataToSend.append('to_email', 'hoshizorahikaru1@gmail.com');
      formDataToSend.append('subject', 'Pendaftaran Bot WhatsApp - VZ Botz Indonesia');
...
Dikirim dari website VZ Botz Indonesia
      ...
      if (result.success) {
        ...
        throw new Error(result.message || 'Gagal mengirim form');
      }
